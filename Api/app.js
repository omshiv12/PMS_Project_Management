...
app.post('/upload', function(req, res) {
  ...
  file1.mv('./images/file1.jpg', function(err) {
    if (err)
      return res.status(500).send(err);
    status['1'] = 'File1 uploaded!';
  });

  file2.mv('./images/file2.jpg', function(err) {
    if (err)
      return res.status(500).send(err);
    status['2'] = 'File2 uploaded!';
    // res.send('File2 uploaded!');
  });

  file3.mv('./images/file3.jpg', function(err) {
    if (err)
      return res.status(500).send(err);
    status['3'] = 'File3 uploaded!';
    // res.send('File3 uploaded!');
  });

  file4.mv('./images/file4.jpg', function(err) {
    if (err)
      return res.status(500).send(err);
    status['4'] = 'File4 uploaded!';
    // res.send('File4 uploaded!');
  });
...
app.get("/update/:collection/:data/:where", (request, response) => {
    ...
    let key = Object.keys(JSON.parse(where))['0'];
    let val = "";
    if(key == "_id"){
        val = ObjectId(JSON.parse(where)['_id']);
    }
...
console.log(data);
    ...
    collection.updateMany(query,update,(function(err, documents) {
        let resp = {};
        if (err){
            resp['status']="Error";
            throw err;
